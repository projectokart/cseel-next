...
export default function OrgClasses() {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      setLoading(true);
      const { data } = await (supabase as any).from("classes")
      ...
      const { data: profiles } = await (supabase as any).from("profiles").select("user_id, display_name").in("user_id", teacherIds);
