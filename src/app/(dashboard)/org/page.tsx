...
export default function OrgDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: orgData } = await (supabase as any).from("organisations").select("*").eq("user_id", user.id).maybeSingle();
      if (orgData) setOrg(orgData);

      const [teachers, students, classes, submissions] = await Promise.all([
        (supabase as any).from("user_roles").select("id", { count: "exact", head: true }).eq("role", "teacher"),
        (supabase as any).from("user_roles").select("id", { count: "exact", head: true }).eq("role", "student"),
        (supabase as any).from("classes").select("id", { count: "exact", head: true }).eq("is_active", true),
        (supabase as any).from("submissions").select("id", { count: "exact", head: true }),
      ]);
      setStats({ teachers: teachers.count || 0, students: students.count || 0, classes: classes.count || 0, submissions: submissions.count || 0 });

      const { data: cls } = await (supabase as any).from("classes").select("id, name, created_at, class_code").eq("is_active", true).order("created_at", { ascending: false }).limit(5);
  ...
  const statCards = [
    { label: "Teachers",    value: stats.teachers,    icon: GraduationCap, color: "text-blue-600",   bg: "bg-blue-50",   href: "/org/teachers" },
    { label: "Students",    value: stats.students,    icon: Users,         color: "text-green-600",  bg: "bg-green-50",  href: "/org/students" },
    { label: "Classes",     value: stats.classes,     icon: BookOpen,      color: "text-purple-600", bg: "bg-purple-50", href: "/org/classes"  },
    { label: "Submissions", value: stats.submissions, icon: TrendingUp,    color: "text-orange-600", bg: "bg-orange-50", href: "/org/reports"  },
  ];
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          ...
          <Link href="/org/classes" className="flex items-center gap-1 text-xs text-primary hover:underline">View all <ArrowRight className="h-3 w-3" /></Link>
        </CardHeader>
