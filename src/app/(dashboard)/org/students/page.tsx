...
export default function OrgStudents() {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      setLoading(true);
      const { data: roles } = await (supabase as any).from("user_roles").select("user_id").eq("role", "student");
      if (!roles || roles.length === 0) { setLoading(false); return; }
      const ids = roles.map(r => r.user_id);
      const { data: profiles } = await (supabase as any).from("profiles").select("*").in("user_id", ids);
      const withEnrollments = await Promise.all((profiles || []).map(async (p) => {
        const { count } = await (supabase as any).from("class_enrollments").select("id", { count: "exact", head: true }).eq("student_id", p.user_id);
        const { count: subs } = await (supabase as any).from("submissions").select("id", { count: "exact", head: true }).eq("student_id", p.user_id);
