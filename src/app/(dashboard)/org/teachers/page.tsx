...
export default function OrgTeachers() {
  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      setLoading(true);
      const { data: roles } = await (supabase as any).from("user_roles").select("user_id").eq("role", "teacher");
      ...
      const withClasses = await Promise.all((profiles || []).map(async (p) => {
        const { count } = await (supabase as any).from("classes").select("id", { count: "exact", head: true }).eq("teacher_id", p.user_id).eq("is_active", true);
