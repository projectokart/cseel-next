...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      const { data: enrollments } = await (supabase as any).from("class_enrollments").select("class_id").eq("student_id", user.id);
      const classIds = enrollments?.map((e) => e.class_id) || [];
      
      let assignments = 0;
      if (classIds.length > 0) {
        const { count } = await (supabase as any).from("assignments").select("id", { count: "exact", head: true }).in("class_id", classIds);
        assignments = count || 0;
      }
      
      const { count: submitted } = await (supabase as any).from("submissions").select("id", { count: "exact", head: true }).eq("student_id", user.id);
  ...
  const handleJoinClass = async () => {
    ...
    // Find class by code
    const { data: classData } = await (supabase as any).from("classes").select("id").eq("class_code", classCode).maybeSingle();
    if (!classData) {
      toast({ title: "Error", description: "Invalid class code", variant: "destructive" });
      return;
    }
    const { error } = await (supabase as any).from("class_enrollments").insert({ class_id: classData.id, student_id: user.id });
    if (error?.code === "23505") {
