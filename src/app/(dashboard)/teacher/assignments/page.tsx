...
export default function TeacherAssignments() {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      const [a, c, e] = await Promise.all([
        (supabase as any).from("assignments").select("*, classes(name), experiments(title)").eq("teacher_id", user.id).order("created_at", { ascending: false }),
        (supabase as any).from("classes").select("id, name").eq("teacher_id", user.id),
        (supabase as any).from("experiments").select("id, title").eq("is_active", true),
      ]);
  ...
  const handleCreate = async () => {
    ...
    // Refetch
    const { data } = await (supabase as any).from("assignments").select("*, classes(name), experiments(title)").eq("teacher_id", user.id).order("created_at", { ascending: false });
