...
export default function TeacherClasses() {
  ...
  const fetchClasses = async () => {
    if (!user) return;
    const { data } = await (supabase as any).from("classes").select("*").eq("teacher_id", user.id).order("created_at", { ascending: false });
    if (data) {
      // Get enrollment counts
      const withCounts = await Promise.all(data.map(async (c) => {
        const { count } = await (supabase as any).from("class_enrollments").select("id", { count: "exact", head: true }).eq("class_id", c.id);
  ...
  const handleCreate = async () => {
    ...
    await (supabase as any).from("classes").insert({ name: form.name, description: form.description || null, teacher_id: user.id });
