...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      const { data: classes } = await (supabase as any).from("classes").select("id").eq("teacher_id", user.id);
      const classIds = classes?.map((c) => c.id) || [];
      
      let students = 0;
      let assignments = 0;
      if (classIds.length > 0) {
        const { count: sc } = await (supabase as any).from("class_enrollments").select("id", { count: "exact", head: true }).in("class_id", classIds);
        const { count: ac } = await (supabase as any).from("assignments").select("id", { count: "exact", head: true }).eq("teacher_id", user.id);
        students = sc || 0;
        assignments = ac || 0;
      }
