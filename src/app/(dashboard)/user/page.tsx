...
export default function UserDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const [profileRes, projectsRes, expsRes] = await Promise.all([
        ...
        (supabase as any).from("user_projects").select("id, title, status, created_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(4),
        (supabase as any).from("experiments").select("id", { count: "exact", head: true }).eq("is_active", true),
      ]);
  ...
  const statCards = [
    { label: "My Projects",    value: stats.projects,    icon: FolderOpen,    href: "/user/projects",    color: "text-blue-600",  bg: "bg-blue-50"  },
    { label: "Lab Experiments", value: stats.experiments, icon: FlaskConical, href: "/simulations",       color: "text-green-600", bg: "bg-green-50" },
  ];
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <Card className="bg-gradient-to-r from-primary/5 to-primary/10 border-primary/20">
        <CardContent className="p-6 flex items-center gap-4">
          ...
          <Link href="/user/profile" className="ml-auto">
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-border">
              {recentProjects.map((p) => (
                <Link key={p.id} href={`/my-project/${p.id}`} className="flex items-center gap-3 px-6 py-3 hover:bg-muted/50 transition-colors">
