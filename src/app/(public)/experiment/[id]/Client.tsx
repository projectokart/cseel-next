...
import {
  ...
  ExternalLink, Clock, Star, Zap, Eye, List,
} from "lucide-react";
import { useCart } from "@/contexts/CartContext";
import LabMaterialDetail, { LabMaterialData } from "@/components/LabMaterialDetail";
import { slugify } from "@/lib/utils";
import { getExperimentBySlugOrId, ALL_EXPERIMENTS } from "@/lib/experimentsData";
...
// ─── SIDEBAR MENU ─────────────────────────────────────────────────────────────
const SIDEBAR_ITEMS = [
  { icon: Target,         label: "Aim & Theory" },
  { icon: BookOpen,       label: "About" },
  { icon: Beaker,         label: "Materials Required" },
  { icon: Package,        label: "Lab Materials" },
  { icon: ListOrdered,    label: "Procedure" },
  { icon: AlertTriangle,  label: "Precautions" },
  { icon: CheckCircle,    label: "Expected Outcome" },
  { icon: PlayCircle,     label: "Images & Video Demo" },
  { icon: Sparkles,       label: "Interactive Animation" },
  { icon: Calculator,     label: "Calculation & Formulae" },
  { icon: BarChart2,      label: "Graph & Plot" },
  { icon: Code2,          label: "Code Verification" },
  { icon: Map,            label: "Mind Map" },
  { icon: HelpCircle,     label: "Theory Questions" },
  { icon: ClipboardList,  label: "MCQ Assessment" },
  { icon: Trophy,         label: "Student Test" },
  { icon: MessageSquare,  label: "Lab Discussion" },
  { icon: Link2,          label: "Related Experiments" },
  { icon: Layers,         label: "Related Projects" },
];

export const scrollToExperimentSection = (label: string) => {
  ...
  if (el) {
    ...
    const headerOffset = navH + 56;
    const y = el.getBoundingClientRect().top + window.scrollY - headerOffset;
    window.scrollTo({ top: y, behavior: "smooth" });
  }
};

// Mobile Off-canvas Table of Contents Drawer
const SidebarMenu = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className="fixed inset-0 bg-black/60 backdrop-blur-xs z-[1300] lg:hidden animate-in fade-in duration-200"
        onClick={onClose}
      />

      {/* Drawer */}
      <div className="fixed top-0 left-0 bottom-0 w-[270px] bg-slate-950 text-white z-[1310] shadow-2xl flex flex-col border-r border-white/10 lg:hidden animate-in slide-in-from-left duration-250">
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-white/10 bg-white/5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-extrabold tracking-wider text-white uppercase">Contents</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 p-2.5 space-y-1">
          {SIDEBAR_ITEMS.map(({ icon: Icon, label }, i) => (
            <button
              key={i}
              onClick={() => {
                scrollToExperimentSection(label);
                onClose();
              }}
              className="w-full flex items-center gap-2.5 px-3 py-2.5 text-left text-white/80 hover:text-white hover:bg-white/10 rounded-xl text-xs font-medium transition-all cursor-pointer"
            >
...
const RelatedExperiments = () => (
  <div className="mt-1">
    <div className="flex items-center justify-between mb-3">
      ...
      <Link href="/hands-on-experiments" className="text-[10px] text-primary font-semibold hover:underline">View All →</Link>
    </div>
...
const RelatedProjects = () => (
  <div className="mt-1">
    <div className="flex items-center justify-between mb-3">
      ...
      <Link href="/projects" className="text-[10px] text-primary font-semibold hover:underline">Browse All →</Link>
    </div>
    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide snap-x snap-mandatory">
      {PROJECTS.map((p) => (
        <div key={p.id}
...
const ExperimentDetail = () => {
  ...
  useEffect(() => {
    if (!id) return;
    const local = getExperimentBySlugOrId(id);
    if (local) {
      setExperiment({
        id: local.id,
        title: local.title,
        ...
        images: local.images,
        materials: local.materials.join("\n"),
        procedure: local.procedure.join("\n"),
        outcome: local.outcome,
        precautions: local.precautions.join("\n"),
        video_link: local.video_link || null,
        demo_link: null,
      });

      // Map related lab materials from local catalog
      if (local.relatedMaterialIds && local.relatedMaterialIds.length > 0) {
        const mats = ALL_MATERIALS.filter(m => local.relatedMaterialIds?.includes(m.id)).map(m => ({
          ...m,
          scientific_name: m.name,
          quantity: "1",
          unit: "Kit / Set",
        }));
        setLabMaterials(mats as any);
      }
    }
    setLoading(false);
  }, [id]);
...
<div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
        ...
        <Link href="/hands-on-experiments" className="flex items-center gap-2 text-primary font-semibold hover:underline text-xs">
  ...
  return (
    <PageTransition>
      <>
        {/* Sticky Breadcrumb with Table of Contents Toggle */}
        <div className="border-b bg-white/95 backdrop-blur sticky top-0 z-20 shadow-xs">
          <div className="w-full py-2 flex justify-center px-4">
            <div className="w-full max-w-4xl flex items-center justify-between gap-3 text-xs min-w-0">
              <div className="flex items-center gap-1.5 min-w-0 truncate">
                <Link href="/hands-on-experiments" className="flex items-center gap-1 text-muted-foreground hover:text-primary font-medium transition-colors shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-1 bg-primary/10 hover:bg-primary text-primary hover:text-white rounded-full font-bold text-[11px] transition-colors shrink-0 cursor-pointer shadow-2xs"
                title={indexOpen ? "Hide Table of Contents" : "Show Table of Contents"}
              >
                <List className="h-3.5 w-3.5" />
                <span>{indexOpen ? "Hide Menu" : "Contents"}</span>
              </button>
            </div>
          </div>
        </div>

        <div className="w-full py-5 flex justify-center px-4 md:px-6">
          <div className="w-full max-w-6xl flex items-start gap-8">

            {/* Dedicated Non-Overlapping Left Column (Desktop) */}
            {indexOpen && (
              <aside className="hidden lg:block w-[240px] shrink-0 sticky top-[68px] z-10 animate-in fade-in slide-in-from-left duration-200">
                <div className="bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 text-white rounded-2xl border border-white/10 shadow-2xl overflow-hidden max-h-[calc(100vh-85px)] flex flex-col">
                  <div className="flex items-center justify-between px-3.5 py-3 border-b border-white/10 bg-white/5">
                    <div className="flex items-center gap-2">
                      <List className="w-4 h-4 text-teal-400" />
                      <span className="text-[11px] font-extrabold tracking-wider text-white uppercase">Contents</span>
                    </div>
                    <button
                      onClick={() => setIndexOpen(false)}
                      className="p-1 rounded-md text-white/70 hover:text-white hover:bg-white/10 transition-colors text-xs flex items-center gap-0.5 font-bold cursor-pointer"
                      title="Hide Menu"
                    >
                      <span className="text-[10px] uppercase font-bold text-gray-300">Hide</span>
                  ...
                  <div className="overflow-y-auto flex-1 p-2 space-y-1">
                    {SIDEBAR_ITEMS.map(({ icon: Icon, label }, i) => (
                      <button
                        key={i}
                        onClick={() => scrollToExperimentSection(label)}
                        className="w-full flex items-center gap-2 px-2.5 py-2 text-left text-white/80 hover:text-white hover:bg-white/10 rounded-xl text-xs font-medium transition-all group cursor-pointer"
                      >
                        <Icon className="w-4 h-4 text-cyan-400 group-hover:text-cyan-300 shrink-0" />
                        <span className="truncate">{label}</span>
                      </button>
                    ))}
                  </div>
          ...
          <div className="mt-6 pt-5 border-t border-border">
            <Link href="/hands-on-experiments"
      ...
      {/* Index Modal / Sidebar (Mobile Drawer) */}
