...
const Materials = () => {
  ...
  useEffect(() => { setVisibleCount(PAGE_SIZE); }, [search, activeCategory, activeSort]);
          ...
          <div className="container mx-auto px-4">
                ...
                {displayed.map((p,i)=>{
                  ...
                  return (
                    <div key={p.id}
                      ...
                      onClick={() => router.push(`/materials/${(p as any).slug || p.id}`)}>
                      {/* Image */}
                        ...
                        <div className="mt-auto space-y-2" onClick={e=>e.stopPropagation()}>
                          {inCart ? (
                            ...
                            <div className="flex items-center gap-1.5">
                              <ShareButton
                                title={p.name || p.scientific_name}
                                text={`Lab Kit: ${p.name || p.scientific_name} - ₹${p.price.toLocaleString('en-IN')}. ${p.category} STEM Kit.`}
                                url={`/materials/${(p as any).slug || p.id}`}
                                size="xs"
                                variant="icon"
                              />
                              <button
                                onClick={() => router.push(`/materials/${(p as any).slug || p.id}`)}
                                className="flex-1 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-800 text-[11px] font-bold rounded-full transition-colors text-center"
                              >
                                Details
                              </button>
                              <button
                                onClick={()=>addToCart(p)}
                                className="flex-1 py-1.5 bg-primary text-white text-[11px] font-bold rounded-full hover:bg-primary/90 transition-colors flex items-center justify-center gap-1"
                              >
            ...
            <div ref={sentinelRef} className="py-10 flex justify-center">
              {visibleCount < filtered.length ? (
                ...
                !loading && filtered.length > 0 && <span className="text-[10px] font-black text-gray-200 uppercase tracking-widest">End of Catalog</span>
              )}
            </div>
          </div>
        </section>
