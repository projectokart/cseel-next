...
const Projects = () => {
  ...
  const [totalCount, setTotalCount] = useState<number | null>(ALL_PROJECTS.length);
  ...
  const fetchPage = useCallback(async (
    ...
    try {
      let filtered = [...ALL_PROJECTS].map(p => ({
        id: p.id,
        title: p.title,
        author: p.author,
        difficulty: p.difficulty,
      ...
      if (difficulties.length > 0) filtered = filtered.filter(p => difficulties.includes(p.difficulty));
      if (tags.length > 0) filtered = filtered.filter(p => tags.some(t => p.tags.includes(t)));

      switch (sort) {
        case "likes":    filtered.sort((a, b) => b.likes - a.likes); break;
        case "rating":   filtered.sort((a, b) => b.rating - a.rating); break;
        case "newest":   filtered.sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime()); break;
        default:         filtered.sort((a, b) => b.views - a.views);
      }

      const count = filtered.length;
      const paginated = filtered.slice(pageNum * PAGE_SIZE, (pageNum + 1) * PAGE_SIZE);

      setProjects(prev => pageNum === 0 ? paginated : [...prev, ...paginated]);
      setTotalCount(count);
      if (paginated.length < PAGE_SIZE || (pageNum + 1) * PAGE_SIZE >= count) {
        setHasMore(false);
      } else {
        setHasMore(true);
      }
  ...
  const createNewProject = async () => {
    ...
    try {
      ...
      router.push(`/my-project/${data.id}`);
    } catch {}
...
{/* Premium Hero Banner */}
        ...
        <div className="bg-white/95 backdrop-blur-md border-b shadow-sm">
          <div className="max-w-screen-xl mx-auto px-4 py-3 flex items-center gap-3">
            ...
            {/* Cart */}
            <button onClick={() => router.push("/cart")} className="relative flex-shrink-0 w-9 h-9 flex items-center justify-center bg-gray-100 rounded-full hover:bg-gray-200 transition-colors">
            ...
            {user && (
              <button onClick={() => router.push("/my-projects")} className="flex-shrink-0 flex items-center gap-1.5 px-3 py-2 bg-primary text-white rounded-full text-xs font-bold hover:bg-primary/90 transition-colors">
                <FolderOpen size={14} />
                My Projects
                ...
                {projects.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5">
                    {projects.map((p, i) => (
                      <div
                        key={p.id}
                        onClick={() => router.push(`/project/${slugify(p.title) || p.id}`)}
                          ...
                          <div className="mt-3 flex gap-2">
                            ...
                            <div onClick={(e) => e.stopPropagation()}>
                              <ShareButton
                                title={p.title}
                                text={p.description ?? ''}
                                url={`/project/${slugify(p.title) || p.id}`}
                                size="sm"
                                variant="icon"
                              />
                            </div>
