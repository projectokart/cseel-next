...
import {
  ...
  X, Check, ExternalLink, Palette, Paperclip, FileText,
  RowsIcon, Columns, Trash2, ArrowUp, ArrowDown,
  ArrowLeft, ArrowRight, CheckSquare, Minus, ListChecks,
  Upload
} from 'lucide-react';
    ...
    <div ref={toolbarRef} className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 p-1.5 sm:p-2 flex items-center justify-between gap-1 flex-wrap text-xs sticky top-0 z-30 select-none">
      ...
      <div className="flex items-center gap-0.5 flex-wrap">
        ...
        <div className="relative">
          <button
            type="button"
            onClick={() => { setHeadingDropdownOpen(!headingDropdownOpen); setInsertDropdownOpen(false); setAiDropdownOpen(false); setTextColorOpen(false); setHighlightColorOpen(false); }}
            ...
            <div className="absolute top-full left-0 mt-1 w-44 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl py-1 z-50">
              {[
                ...
                <button
                  key={item.label}
                  type="button"
                  onClick={() => { item.action(); setHeadingDropdownOpen(false); }}
                  className={`w-full px-3 py-1.5 text-left hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-between ${item.style} ${
                    item.check ? 'text-purple-600 bg-purple-50 dark:bg-purple-950' : 'text-slate-700 dark:text-slate-300'
                  }`}
                >
                  {item.label}
                  {item.check && <Check className="w-3.5 h-3.5" />}
                </button>
              ))}
            </div>
          )}
        ...
        <div className="relative border-l border-slate-200 dark:border-slate-800 pl-1">
          <button
            type="button"
            onClick={() => { setTextColorOpen(!textColorOpen); setHighlightColorOpen(false); setInsertDropdownOpen(false); }}
            ...
            title="Text Color"
          >
            <span className="text-xs font-black" style={{ color: editor.getAttributes('textStyle').color || '#374151' }}>A</span>
            <div className="w-3.5 h-1 rounded-full" style={{ background: editor.getAttributes('textStyle').color || '#374151' }} />
          </button>
        ...
        <div className="relative">
          <button
            type="button"
            onClick={() => { setHighlightColorOpen(!highlightColorOpen); setTextColorOpen(false); setInsertDropdownOpen(false); }}
          ...
          {highlightColorOpen && (
            <div className="absolute top-full left-0 mt-1 p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-50 w-[180px]">
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {HIGHLIGHT_COLORS.map((c) => (
                  <button
                    ...
                    className="w-8 h-5 rounded-md border border-slate-200 hover:border-purple-500 hover:scale-110 transition-all text-[8px] font-bold text-slate-600"
              ...
              <button
                type="button"
                onClick={() => { editor.chain().focus().unsetHighlight().run(); setHighlightColorOpen(false); }}
        ...
        {editor.isActive('table') && (
          <div className="relative border-l border-slate-200 dark:border-slate-800 pl-1">
            <button
              type="button"
              onClick={() => { setTableMenuOpen(!tableMenuOpen); setInsertDropdownOpen(false); }}
            ...
            {tableMenuOpen && (
              <div className="absolute top-full left-0 mt-1 w-52 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl py-1 z-50">
                ...
                <button type="button" onClick={() => { editor.chain().focus().addRowBefore().run(); setTableMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100">
                  <ArrowUp className="w-3.5 h-3.5 text-teal-600" /> Add Row Above
                </button>
                <button type="button" onClick={() => { editor.chain().focus().addRowAfter().run(); setTableMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100">
                  <ArrowDown className="w-3.5 h-3.5 text-teal-600" /> Add Row Below
                </button>
                <button type="button" onClick={() => { editor.chain().focus().deleteRow().run(); setTableMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-50">
                ...
                <button type="button" onClick={() => { editor.chain().focus().addColumnBefore().run(); setTableMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100">
                  <ArrowLeft className="w-3.5 h-3.5 text-teal-600" /> Add Column Left
                </button>
                <button type="button" onClick={() => { editor.chain().focus().addColumnAfter().run(); setTableMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100">
                  <ArrowRight className="w-3.5 h-3.5 text-teal-600" /> Add Column Right
                </button>
                <button type="button" onClick={() => { editor.chain().focus().deleteColumn().run(); setTableMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-50">
                  <Trash2 className="w-3.5 h-3.5" /> Delete Column
                </button>

                <div className="border-t border-slate-100 dark:border-slate-800 my-1" />
                <p className="px-3 py-1 text-[9px] font-black uppercase text-slate-400">Table</p>
                <button type="button" onClick={() => { editor.chain().focus().toggleHeaderRow().run(); setTableMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100">
                  <RowsIcon className="w-3.5 h-3.5 text-indigo-600" /> Toggle Header Row
                </button>
                <button type="button" onClick={() => { editor.chain().focus().mergeCells().run(); setTableMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100">
                  <Columns className="w-3.5 h-3.5 text-indigo-600" /> Merge Cells
                </button>
                <button type="button" onClick={() => { editor.chain().focus().splitCell().run(); setTableMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100">
                  <Columns className="w-3.5 h-3.5 text-indigo-600" /> Split Cell
                </button>
                <button type="button" onClick={() => { editor.chain().focus().deleteTable().run(); setTableMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-50">
        ...
        <div className="relative border-l border-slate-200 dark:border-slate-800 pl-1">
          <button
            type="button"
            onClick={() => { setInsertDropdownOpen(!insertDropdownOpen); setHeadingDropdownOpen(false); setAiDropdownOpen(false); setTextColorOpen(false); setHighlightColorOpen(false); }}
          ...
          {insertDropdownOpen && (
            <div className="absolute top-full left-0 mt-1 w-64 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl py-1 z-50 max-h-[400px] overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800">
              ...
              <div className="p-1 space-y-0.5">
                ...
                {config.tables && (
                  <button type="button" onClick={() => { editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run(); setInsertDropdownOpen(false); }}
                ...
                {/* Image Upload */}
                <button type="button" onClick={() => { setImageDialogOpen(true); setInsertDropdownOpen(false); }}
                  className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-slate-100 text-left text-xs font-bold text-slate-700">
                  <ImageIcon className="w-3.5 h-3.5 text-blue-600" /> Image (Upload / URL)
                </button>
                {/* File/PDF Attachment */}
                <button type="button" onClick={() => { fileInputRef.current?.click(); setInsertDropdownOpen(false); }}
                  className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-slate-100 text-left text-xs font-bold text-slate-700">
                  <Paperclip className="w-3.5 h-3.5 text-orange-600" /> File / PDF Attachment
                </button>
                {/* Horizontal Rule */}
                <button type="button" onClick={() => { editor.chain().focus().setHorizontalRule().run(); setInsertDropdownOpen(false); }}
                  className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-slate-100 text-left text-xs font-bold text-slate-700">
                  <Minus className="w-3.5 h-3.5 text-slate-500" /> Horizontal Divider
                </button>
                {/* Code Block */}
                <button type="button" onClick={() => { editor.chain().focus().toggleCodeBlock().run(); setInsertDropdownOpen(false); }}
                  className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-slate-100 text-left text-xs font-bold text-slate-700">
              ...
              {(config.formulas || config.graphs || config.diagrams) && (
                <div className="p-1 space-y-0.5">
                  ...
                  {config.formulas && (
                    <button type="button" onClick={() => { editor.chain().focus().insertContent({ type: 'formula', attrs: { latex: 'V = I \\cdot R' } }).run(); setInsertDropdownOpen(false); }}
                      className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-slate-100 text-left text-xs font-bold text-slate-700">
                      <FunctionSquare className="w-3.5 h-3.5 text-purple-600" /> LaTeX Formula
                    </button>
                  )}
                  {config.graphs && (
                    <button type="button" onClick={() => { editor.chain().focus().insertContent({ type: 'graph' }).run(); setInsertDropdownOpen(false); }}
                      className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-slate-100 text-left text-xs font-bold text-slate-700">
                      <TrendingUp className="w-3.5 h-3.5 text-[#006fcc]" /> Interactive Graph
                    </button>
                  )}
                  {config.diagrams && (
                    <button type="button" onClick={() => { editor.chain().focus().insertContent({ type: 'mermaid' }).run(); setInsertDropdownOpen(false); }}
                      className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-slate-100 text-left text-xs font-bold text-slate-700">
              ...
              {config.callouts && (
                <div className="p-1 space-y-0.5">
                  ...
                  {[
                    ...
                    <button key={c.variant} type="button" onClick={() => {
      ...
      <div className="flex items-center gap-1">
            ...
            <div className="absolute top-full right-0 mt-1 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl p-3 z-50">
              ...
              <div className="flex items-center gap-2">
                ...
                {editor.isActive('link') && (
                  <button type="button" onClick={() => { editor.chain().focus().unsetLink().run(); setLinkPopoverOpen(false); }}
        ...
        {config.aiAssistance && (
          <div className="relative">
            <button type="button" onClick={() => { setAiDropdownOpen(!aiDropdownOpen); setInsertDropdownOpen(false); setHeadingDropdownOpen(false); }}
            ...
            {aiDropdownOpen && (
              <div className="absolute top-full right-0 mt-1 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl py-1 z-50">
                ...
                <div className="py-1">
                  {AI_ACTIONS.map((item) => (
                    <button key={item.action} type="button" onClick={() => { onApplyAI?.(item.action); setAiDropdownOpen(false); }}
