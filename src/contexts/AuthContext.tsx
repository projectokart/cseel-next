...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchRoles = async (userId: string) => {
    ...
    try {
      const { data, error } = await (supabase as any).from("user_roles")
        .select("role")
        .eq("user_id", userId);
  ...
  const signUp = async (
    ...
    if (!error && data.user) {
      ...
      // Insert role
      const { error: roleError } = await (supabase as any).from("user_roles")
      ...
      await (supabase as any).from("profiles").update(profileUpdate).eq("user_id", userId);
  ...
  const resetPassword = async (email: string) => {
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${typeof window !== 'undefined' ? (typeof window !== 'undefined' ? window.location.origin : (process.env.NEXT_PUBLIC_SITE_URL || '')) : (process.env.NEXT_PUBLIC_SITE_URL || '')}/reset-password`,
    });
    return { error };
  };
