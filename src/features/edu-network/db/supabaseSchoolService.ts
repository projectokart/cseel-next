...
export class SupabaseSchoolService {
  ...
  public mapDbToOrg(row: any): OrganizationItem {
    ...
    return {
      ...
      slug: row.slug,
      name: row.name,
      shortName: row.short_name || detail.shortName,
      type: row.org_type || detail.orgType || 'School',
      affiliation: row.affiliation || detail.affiliation,
      city: row.city,
      state: row.state,
      pincode: row.pincode,
      address: row.address,
      ...
      block: row.block || detail.block || '',
      villageTownCity: row.village_town_city || detail.villageTownCity || row.city,
      email: row.email || detail.email,
      phone: row.phone || detail.phone,
      website: row.website || detail.website,
      verified: row.verified ?? true,
      rating: Number(row.rating) || 4.9,
      reviews: Number(row.reviews_count) || 142,
      stemLabsCount: Number(row.stem_labs_count) || (detail.stemLabsList?.length || 12),
      studentStrength: Number(row.student_strength) || (detail.studentStrength || 2400),
      logo: row.logo_url || 'https://images.unsplash.com/photo-1580582932707?w=200&auto=format&fit=crop',
      bannerImage: row.banner_url || 'https://images.unsplash.com/photo-1509062522246?w=800&auto=format&fit=crop',
      description: row.description || detail.description || '',
      openJobsCount: row.open_jobs_count || detail.careersList?.length || 1,
      established: Number(row.established_year) || detail.establishedYear || 1998,
      facilities: row.facilities_chips || detail.facilitiesChips || ['Smart Classrooms', 'Robotics Lab', 'Transport'],
      classesOffered: row.classes_offered || detail.classesOffered || 'Pre-K - 12th',
      monthlyFees: row.monthly_fees || detail.monthlyFees || '₹12,500 / mo',
      monthlyFeesNum: Number(row.monthly_fees_num) || detail.monthlyFeesNum || 12500,
      board: row.board || detail.board || 'CBSE',
      studentFacultyRatio: row.student_faculty_ratio || detail.studentFacultyRatio || '13:1',
      admissionStatus: row.admission_status || detail.admissionStatus || 'Open for 2026-27',
      udiseCode: row.udise_code || detail.udiseCode || '07010200389',
      isFeatured: row.is_featured ?? false,
      
      // Detail Page Nested Entities
      ...detail,
      ...row,
      principalName: detail.principalName || row.principal_name || 'Dr. Sunita Kapoor',
      principalDesignation: detail.principalDesignation || row.principal_designation || 'Principal (Ph.D, M.Ed)',
      principalPhoto: detail.principalPhoto || row.principal_photo || 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&w=120&h=120&q=80',
      principalMessage: detail.principalMessage || row.principal_message || '"We inspire holistic global education by instilling critical inquiries, creativity, and empathy in every learner."',
      galleryPhotos: detail.galleryPhotos || row.gallery_photos || [],
      videosList: detail.videosList || row.videos_list || [],
      contactChannels: detail.contactChannels || row.contact_channels || [],
      selectedLabs: detail.stemLabsList || row.selected_labs || [],
      requiredDocs: detail.requiredDocs || row.required_docs || [],
      feeBreakdown: detail.feeBreakdown || row.fee_breakdown || {},
      classWiseFees: detail.classWiseFees || row.class_wise_fees || {},
      facilitiesMatrix: detail.facilitiesMatrix || row.facilities_matrix || {},
      academicResults: detail.academicResults || row.academic_results || {},
      parentInsights: detail.parentInsights || row.parent_insights || {},
      careersList: detail.careersList || row.careers_list || [],
    };
  }
