...
export const postsApi = {
  getFeed: async (type?: PostType, limit = 20, offset = 0) => {
    let q = (supabase as any).from("posts")
 ...
 create: async (payload: Partial<Post> & { author_id: string }) => {
  ...
  delete: async (id: string) => (supabase as any).from("posts").delete().eq("id", id),

  getLikedPosts: async (userId: string) => {
    const { data } = await (supabase as any).from("post_likes").select("post_id").eq("user_id", userId);
    return new Set((data || []).map(l => l.post_id));
  },

  like: async (postId: string, userId: string) => {
    return (supabase as any).from("post_likes").insert({ post_id: postId, user_id: userId });
  },

  unlike: async (postId: string, userId: string) => {
    return (supabase as any).from("post_likes").delete().eq("post_id", postId).eq("user_id", userId);
  },

  getComments: async (postId: string) => {
    const { data, error } = await (supabase as any).from("post_comments")
      .select("*")
      .eq("post_id", postId)
  ...
  addComment: async (postId: string, authorId: string, content: string) => {
    return (supabase as any).from("post_comments").insert({ post_id: postId, author_id: authorId, content }).select().single();
  },

  deleteComment: async (id: string) => (supabase as any).from("post_comments").delete().eq("id", id),
};

// ── Channels ─────────────────────────────────────────────────────────────────
export const channelsApi = {
  getAll: async () => {
    const { data, error } = await (supabase as any).from("channels")
      .select("*")
  ...
  create: async (payload: { owner_id: string; name: string; description?: string; avatar_url?: string; banner_url?: string }) => {
    return (supabase as any).from("channels").insert(payload).select().single();
  },

  getMyChannel: async (userId: string) => {
    return (supabase as any).from("channels").select("*").eq("owner_id", userId).maybeSingle();
  },

  getSubscribedChannels: async (userId: string) => {
    const { data } = await (supabase as any).from("channel_subscribers").select("channel_id").eq("user_id", userId);
    return new Set((data || []).map(s => s.channel_id));
  },

  subscribe: async (channelId: string, userId: string) => {
    return (supabase as any).from("channel_subscribers").insert({ channel_id: channelId, user_id: userId });
  },

  unsubscribe: async (channelId: string, userId: string) => {
    return (supabase as any).from("channel_subscribers").delete().eq("channel_id", channelId).eq("user_id", userId);
  },
